(function(root, factory){
  ...
  function storedSession(storage, key){
    try{
      ...
      return { code: cleanCode(value.code), token: value.token, playerId: String(value.playerId || ""), seat: Number(value.seat) || 0, username: cleanUsername(value.username), transport };
  ...
  function createRoomClient(options){
    ...
    async function resume(){
      ...
      try{
        ...
        return acceptJoin(await request("/api/arcade/rooms/" + encodeURIComponent(prior.code) + "/join", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ username: prior.username || "Player", reconnectToken: prior.token }) }), prior.username || "Player", prior.transport);
      }
      catch(error){ if(error && (error.status === 401 || error.status === 403 || error.status === 404 || error.status === 410)) forget(); throw error; }
      finally{ requestBusy = false; }
    }
