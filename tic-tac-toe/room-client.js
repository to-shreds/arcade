(function(root, factory){
  ...
  function storedSession(storage, key){
    try{
      ...
      return { code: cleanCode(value.code), token: value.token, playerId: String(value.playerId || ""), seat: Number(value.seat) || 0, username: cleanUsername(value.username) };
  ...
  function createRoomClient(options){
    ...
    let session = null, room = null, socket = null, stopped = true, retryTimer = 0, pollTimer = 0, retryCount = 0, requestBusy = false;

    function emitStatus(kind, text){ onStatus({ kind, text: String(text || "") }); }
    function persist(){
      try{ if(session) storage.setItem(sessionKey, JSON.stringify(session)); else storage.removeItem(sessionKey); }catch(_error){}
    ...
    async function request(path, init, timeoutMs){
      ...
      try{
        ...
        if(!response.ok || !body || body.ok === false){ const error = new Error(body && body.error ? body.error : ("Request failed (" + response.status + ")")); error.status = response.status; throw error; }
        return body;
      }catch(error){
        if(error && error.name === "AbortError"){ const timeout = new Error("The room server did not respond in time."); timeout.status = 408; throw timeout; }
        throw error;
    ...
    function closeSocket(){
      ...
      if(socket){ const old = socket; socket = null; try{ old.onopen = old.onclose = old.onerror = old.onmessage = null; old.close(1000, "Leaving room"); }catch(_error){} }
    }
    function schedulePoll(delay){
      if(stopped || !session) return;
      if(pollTimer) clearTimeout(pollTimer);
      pollTimer = setTimeout(async function(){ pollTimer = 0; try{ await refresh(); }catch(_error){} if(!socket || socket.readyState !== 1) schedulePoll(3000); }, delay == null ? 0 : delay);
    }
    function scheduleReconnect(){
      ...
      schedulePoll(0);
      retryTimer = setTimeout(function(){ retryTimer = 0; connectSocket(); }, Math.min(8000, 500 * Math.pow(2, Math.min(4, retryCount - 1))));
    }
    function connectSocket(){
      ...
      let opened = false;
      try{ socket = new WebSocketImpl(wsBase + "/api/arcade/rooms/" + encodeURIComponent(session.code) + "/ws?token=" + encodeURIComponent(session.token)); }
    ...
    function acceptJoin(body, username){
      session = { code: cleanCode(body.code || (body.room && body.room.code)), token: String(body.token || ""), playerId: String(body.playerId || ""), seat: Number(body.seat) || 0, username: cleanUsername(username) };
    ...
    async function join(params){
      if(requestBusy) throw new Error("A room request is already in progress.");
      requestBusy = true;
      const username = cleanUsername(params && params.username), code = cleanCode(params && params.code);
      if(!username){ requestBusy = false; throw new Error("Enter a username first."); }
    ...
    async function resume(){
      ...
      requestBusy = true; emitStatus("connecting", "Rejoining room " + prior.code + "…");
      try{ return acceptJoin(await request("/api/arcade/rooms/" + encodeURIComponent(prior.code) + "/join", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ username: prior.username || "Player", reconnectToken: prior.token }) }), prior.username || "Player"); }
      catch(error){ if(error && (error.status === 401 || error.status === 403 || error.status === 404)) forget(); throw error; }
      finally{ requestBusy = false; }
    }
    ...
    function forget(){ stopped = true; closeSocket(); session = null; room = null; persist(); emitStatus("offline", "Online room closed on this device."); }
    function disconnect(){ stopped = true; closeSocket(); }

    return { create, join, resume, refresh, action, leave, forget, disconnect, saved, cleanCode, cleanUsername, get room(){ return room; }, get session(){ return session ? Object.assign({}, session) : null; } };
  }
