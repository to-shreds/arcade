  ...
  for(let level=1;level<=4;level++){
    ...
    for(let i=0;i<80;i++){
      ...
      if(mode==="hunt"){
        ...
        assert.equal(c.choices.length,4);assert(c.answer>=0&&c.answer<4);assert(E.checkAnswer(c,c.answer));
        ...
        assert.equal(c.choices.length,4);assert(c.broken.includes("___"));assert.equal(c.broken.replace("___",c.patch),c.pattern);assert(E.checkAnswer(c,c.answer));
      }else{
